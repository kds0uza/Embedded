...
if(stop_count >= 28)				 //STOP bot if trying to navigate near end of the hallway  
{
...
if(zone == 0)						 //too close to obstacle, REVERSE & correct
{
		...
		if(left_sensor <= 50)				 // if closer to left wall, then REVERSE with a RIGHT TURN
		{
			while(!(IFG2 & UCA0TXIFG)) {};   // Confirm that Tx Buff is empty
		...
		else								 // if closer to right wall, then REVERSE with a LEFT TURN
		{
			while(!(IFG2 & UCA0TXIFG)) {};   // Confirm that Tx Buff is empty
...
if(front_sensor <= 150)				 //obstacle detected in Zones 2 or 3
{
	if(zone == 2)					 //if obstacle is detected in Zone 2
	{
			...
			// proportional drive = adj + min change + ((distance from obstacle)*(max change - min change)/(difference in boundary limits))
			
			prop = adj + 7 + ((150 - front_sensor)/21);			  // proportional drive = adj + min change + ((distance from obstacle)*(12-7)/(150-45);
...
if(zone == 2)						 //Minor RIGHT
{
	...
	prop = adj + 2 + ((125 - left_sensor)/20);			  // proportional drive = adj + min change + ((distance from zone boundary)*(8-3)/(125-25));
	...
	if(prev_z ==0)
		stop_count++;						  // INCREMENT Stop count if previous zone is Zone 0
...
//if(front_sensor <=150)					//obstacle in front or right of bot, TURN LEFT
	...
	for(j = 0; j <= delay; j++)
	{
		...
		prop = adj + 3 + ((left_sensor - 125)/20);			  // proportional drive = adj + min change + ((distance from zone boundary)*(8-3)/(125-25));
...
if(zone == 1)                        //TURN HARD RIGHT
{
	...
	prop = adj + 3 + ((100 - left_sensor)*10)/75;			  // proportional drive = adj + min change + ((distance from zone boundary)*(13-3)/(100-25));
...
if(zone == 4)						 //TURN HARD LEFT
{
	...
	prop = adj + 3 + ((left_sensor - 125)*10)/75;			  // proportional drive = adj + min change + ((distance from zone boundary)*(13-3)/(200-125));
