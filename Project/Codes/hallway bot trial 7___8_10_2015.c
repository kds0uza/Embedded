...
if(cal/4)
{
if(front_sensor > 120 && left_sensor > 95 && left_sensor < 155)   //moving forward when there is no obstacle or wall ahead
{
	left_motor = 100;
	right_motor = 228;
...
if(front_sensor > 40 && left_sensor >= 250)			  // if new pathway is detected, HARD LEFT	   
{
	  left_motor = 100;
	  right_motor = 228;
...
if(front_sensor <= 120 && left_sensor <= 95)			// if obstacle ahead and bot is too close to left wall, TURN RIGHT
{
	//turn right
	left_motor = 100;
	right_motor = 228;
	
	prop = 18 + (95- left_sensor)*(25 - 18 )/(95 - 25);
	...
	for(i=0;i <= prop/2; i++)							 //GO STRAIGHT
	{
	...
	for(i=0;i <= prop; i++)								 //TURN LEFT AGAIN
	{
...
if(front_sensor <= 120 && left_sensor >= 155)		  //if obtstacle ahead and bot too close to right wall, TURN LEFT
{
	//turn left
	left_motor = 100;
	right_motor = 228;
	
	prop = 18 + (left_sensor - 155)*(25 - 18 )/(200 - 155);
	...
	for(i=0;i <= prop/2; i++)				 //GO STRAIGHT
	{
	 ...
	 for(i=0;i <= prop; i++)				//TURN RIGHT AGAIN 
	{
...
if(front_sensor < 120 && left_sensor > 95 && left_sensor < 150) // if obstacle ahead
{
  left_motor = 100;
	right_motor = 228;	
	
  if(left_sensor < 125)                           //if closer to left wall then Turn Right
 {
	prop = 15 + (125 - left_sensor)*(25 - 15)/(125-95); 
  ...
  if(left_sensor >=125 )                      //if closer to right wall then Turn Left
  {
	prop = 15 + (left_sensor - 125)*(25 - 15)/(155-125); 
...
if(front_sensor > 120 && left_sensor < 95)			   //if all clear ahead but bot is close to left wall, TURN RIGHT
{
	//TURN RIGHT
	...
	prop = 15 + (95 - left_sensor)*(25 - 15)/(95-25);
...
if(front_sensor > 120 && left_sensor > 150 && left_sensor < 200) //if all clear ahead but bot is close to right wall, TURN LEFT 
{
	left_motor = 100;
	right_motor = 228;
	
	prop = 18 + (left_sensor - 100)*(25 - 18)/(200 - 100); 
...
if((front_sensor <= 40 || front_sensor > 650) && left_sensor < 30)			   //if near wall
{
		//left_motor = 20;
		//right_motor =136;
